...
class Step{
    ...
    #透過 job_id 及 seq_id 取得當前有幾個step - Tòuguò job_id jí seq_id qǔdé dāngqián yǒu jǐ gè step
    ...
    public function create_step($step_data) {
        ...
        $statement->bindValue(':step_enable_threshold', $step_data['StepEnbaleThreshold']);
    ...
    public function update_step_by_id($step_data){        
        ...
        $sql = "UPDATE `STEP_lst` SET 
                    ...
                    StepEnbaleThreshold = :step_enable_threshold,
    ...
    public function getStepsWithThresholds($jobid, $seqid) {
        ...
        $sql = "SELECT step_id, StepEnbaleThreshold FROM STEP_lst WHERE JOBID = ? AND SEQID = ?";
        $stmt = $this->db_iDas->prepare($sql);
        $stmt->execute([$jobid, $seqid]);
    ...
    public function resetThresholdsByStepIds($jobid, $seqid, $stepIds) {
        ...
        $sql = "UPDATE STEP_lst SET StepEnbaleThreshold = 0, StepTorqueTS = 0
    ...
    public function resetAllThresholds($jobid, $seqid) {
        $sql = "UPDATE STEP_lst SET StepEnbaleThreshold = 0, StepTorqueTS = 0 WHERE JOBID = ? AND SEQID = ?";
        $stmt = $this->db_iDas->prepare($sql);
        $stmt->execute([$jobid, $seqid]);
        return true;
    }
    ...
    public function resetAllOtherThresholds($jobid, $seqid, $keep_step_select) {
        $sql = "UPDATE STEP_lst 
                SET StepEnbaleThreshold = 0, StepTorqueTS = 0
    ...
    public function getPreviousStepsWithThreshold($jobid, $seqid, $current_step_id) {
        $sql = "SELECT StepSelect FROM STEP_lst
                WHERE JOBID = ? AND SEQID = ? AND StepSelect <> ?
                AND StepEnbaleThreshold > 0 AND StepTorqueTS > 0";
        $stmt = $this->db_iDas->prepare($sql);
