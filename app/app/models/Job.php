...
class Job{
    ...
    public function copy_sequence_by_job_id($new_temp_seq) {
        ...
        $sql = "INSERT INTO SEQ_lst (
                    ...
                    image, message, delay, event_id, input_pin_no, output_pin_no, wave, wave_on, addtion
                ) 
                VALUES (
                    :JOBID, :SEQID, :SEQname, :type, :time, :act, :skip, :seq_repeat,
                    :ok_seq, :ok_stop, :countType, :ok_screw, :unscrew_count, :ng_stop, :ng_unscrew, :interrupt_alarm, 
        ...
        foreach ($new_temp_seq as $seq) {
            try {
                if ($statement->execute([
                        ...
                        ':unscrew_count' => $seq['unscrew_count'],
                        ':ng_stop' => $seq['ng_stop'],
                        ...
                        ':input_pin_no' => $seq['input_pin_no'],
                        ':output_pin_no' => $seq['output_pin_no'],
                        ':wave' => $seq['wave'],
                        ':wave_on' => $seq['wave_on'],
                        ':addtion' => $seq['addtion'],
                    ])) {
    ...
    public function delete_input_by_job_id($new_jobid) {
        #查詢資料是否存在
        $sql = "SELECT COUNT(*) FROM ntcs_io_input WHERE job_id = ?";
        $statement = $this->db_iDas->prepare($sql);
        $statement->execute([$new_jobid]);
        $count = $statement->fetchColumn();
        $count = intval($count);
        ...
        if ($count > 0) {
            #如果資料存在，則刪除
            $deleteSql = "DELETE FROM ntcs_io_input  WHERE job_id = ? ";
            $deleteStatement = $this->db_iDas->prepare($deleteSql);
            $deleteStatement->execute([$new_jobid]);
    ...
    public function delete_output_by_job_id($new_jobid) {
        #查詢資料是否存在
        $sql = "SELECT COUNT(*) FROM ntcs_io_output WHERE job_id = ?";
        $statement = $this->db_iDas->prepare($sql);
        $statement->execute([$new_jobid]);
        $count = $statement->fetchColumn();
        $count = intval($count);
        ...
        if ($count > 0) {
            #如果資料存在，則刪除
            $deleteSql = "DELETE FROM ntcs_io_output  WHERE job_id	 = ? ";
            $deleteStatement = $this->db_iDas->prepare($deleteSql);
            $deleteStatement->execute([$new_jobid]);
     ...
     */
    public function check_type_by_output($ids): array{
        ...
        try {
            ...
            if (empty($ids)) {
                // 沒有要保留的 ID：把全表中 output_unified==1 的都清成 0
                $pdo->exec("UPDATE JOB_lst SET output_unified = 0 WHERE output_unified = 1");
                ...
                $sqlUpdate = "UPDATE JOB_lst
                            SET output_unified = 0
                            WHERE output_unified = 1 AND JOBID NOT IN ($ph)";
            ...
            error_log('check_type_by_output reset failed: '.$e->getMessage());
            return [];
        }
    }
    ...
    public function getUnifiedJobId_by_output(): ?int{
    ...
    public function getUnifiedJobId_by_input(): ?int{
