...
class Datas{
    ...
    public function get_data_for_year() {
        ...
        // Lấy PDO object từ Database class
        $db_data = $database->getDb_data();

        // Kiểm tra kết nối
        if (!$db_data) {
            throw new Exception("Database connection (db_data) is null");
        }

        // Query giữ nguyên tên bảng/cột như dự án
        $sql = "SELECT strftime('%Y', data_time) AS year, COUNT(*) AS total 
                FROM 'data' 
                GROUP BY year 
                ORDER BY year ASC";
    ...
    //     $sql = "SELECT strftime('%Y', data_time) AS year, COUNT(*) AS total FROM 'data' GROUP BY year ORDER BY year ASC";
    //     $statement = $this->db_data->prepare($sql);
    //     if($statement != false){
    //         $statement->execute();
    //         $rows = $statement->fetchAll(PDO::FETCH_ASSOC);
