...
class Steps extends Controller
{
    ...
    public function index($job_id,$seq_id){
        ...
        if($isMobile){
            $this->view('steps/index_m', $data);
        }else{
            $this->view('steps/index', $data);
        }
        ...
        if(isset($_POST['jobid'])){
            ...
            if(!empty($old_res)){
                $step_data = array(
                    ...
                    'StepEnbaleThreshold'      => $old_res[0]['StepEnbaleThreshold'],
    ...
    public function variation($job_id = null, $seq_id = null, $stepid = null){
        ...
        $tools['check_target_tor_lo'] = $tools['check_target_tor_lo'] ?? '';
        $tools['check_target_tor_hi'] = $tools['check_target_tor_hi'] ?? '';
        ...
        if (!empty($tools_check) && $type === "edit") {
            ...
            // 執行轉換與檢查 - Zhí háng zhuǎnhuàn yǔ jiǎnchá
        ...
        if ($isMobile) {
            $this->view('steps/step_form_m', $data);
        } else {
            $this->view('steps/step_form', $data);
        }
    }
    ...
    public function check_step_limit() {
        ...
        // 遍歷每一筆 step，檢查是否有 StepEnbaleThreshold 不為 0 的情況
        foreach ($steps as $step) {
            if ((int)$step['StepEnbaleThreshold'] !== 0) {
                $hasNonZeroThreshold = true;
                $updateIds[] = $step['step_id']; 
            }
        }

        // 若有不為 0 的情況（將 StepEnbaleThreshold 和 StepTorqueTS 設為 0）
        if (!empty($updateIds)) {
