<script>
    ...
    function updateLabel() {
        ...
        // 🕒 Thêm phần dịch “Second” (Time Unit)
        const timeUnits = {
            'zh-cn': '秒',
            'zh-tw': '秒',
            'default': 'Sec'
        };
        const translatedTimeUnit = timeUnits[language] || 'Sec'; // dùng theo ngôn ngữ

        // 標籤文字翻譯 - Biāoqiān wénzì fānyì
        const labelTexts = {
            'zh-cn': { 1: '目标扭矩', 2: '目标角度', 3: '目标时间' },
            'zh-tw': { 1: '目標扭力', 2: '目標角度', 3: '目標時間' },
            'default': { 1: 'Target Torque', 2: 'Target Angle', 3: 'Target Time' }
        };

        const textSet = labelTexts[language] || labelTexts['default'];
        const labelPrefix = textSet[selectVal] || 'Target';

        // 更新標籤內容 - Gēngxīn biāoqiān nèiróng
        if (selectVal === 1) {
            label.textContent = `${labelPrefix} (${translatedUnit}):`; // Torque
        } else if (selectVal === 2) {
            label.textContent = `${labelPrefix} (°):`; // Angle
        } else if (selectVal === 3) {
            label.textContent = `${labelPrefix} (${translatedTimeUnit}):`; // Time
        }
        
        // 隱藏所有主區塊 - Yǐncáng suǒyǒu zhǔ qū kuài
        ['StepTorque_item', 'StepAngle_item', 'StepTime_item'].forEach(id => {
            document.getElementById(id)?.style.setProperty('display', 'none');
        });

        // 顯示對應主區塊 - Xiǎnshì duìyìng zhǔ qū kuài
        if (selectVal === 1){
            ...
            document.getElementById('StepTime_item')?.style.setProperty('display', 'block');
        } 

        // 額外顯示區塊 - Éwài xiǎnshì qū kuài
        const showTor = document.getElementById('show_tor');
        const showAng = document.getElementById('show_ang');
        if (showTor && showAng) {
            showTor.style.display = selectVal === 1 ? 'block' : 'none';
            showAng.style.display = selectVal === 2 ? 'block' : 'none';
        }

        // 控制 StepMoniByWin 及 step_limit 欄位啟用
        const enableMap = {
            1: ['StepMoniByWin_0', 'step_limit_hi_tor', 'step_limit_lo_tor'],
            2: ['StepMoniByWin_1', 'step_limit_hi_ang', 'step_limit_lo_ang'],
           //3: ['StepTime'] // thêm Time
        };
        const allFields = [
        ...
        // 控制 Hi/Lo Torque 與 Angle 欄位啟用 - Kòngzhì Hi/Lo Torque yǔ Angle lán wèi qǐyòng

        const extraFieldMap = {
            1: ['StepHiTorque', 'StepLoTorque'],
            2: ['StepHiAngle', 'StepLoAngle']
        };
        ...
        // 自動勾選未勾選的 StepMoniByWin_X - Zìdòng gōu xuǎn wèi gōu xuǎn de
        if (selectVal === 2) { // angle
    ...
    function toggleStepTorqueTS() {
        ...
        //整塊顯示控制 - Zhěng kuài xiǎnshì kòngzhì
        if (thresholdBlock) {
    ...
    function toggleDownShift() {
        ...
        //取得 torque unit 對應的小數位數 - Qǔdé torque unit duìyìng de xiǎoshù wèi shù
        const stepUnit = parseInt(document.getElementById('step_torque_unit')?.value ?? 1);
        const decimals = {
            0: 2,
            1: 3,
    ...
    if (dataType === 'new') {
        ...
        document.getElementById("StepTime").value = 0;
    ...
    function save_or_edit_step(isEdit = false) {
        ...
        let StepTime = document.getElementById("StepTime")?.value || 0;

        // Xử lý loại dữ liệu không cần thiết
        if (StepOption == 1) {        // Torque
            StepAngle = 0;
            StepTime = 0;
        } else if (StepOption == 2) { // Angle
            StepTorque = 0;
            StepTime = 0;
        } else if (StepOption == 3) { // Time
            StepTorque = 0;
            StepAngle = 0;
        }
        ...
        let StepEnbaleThreshold = document.querySelector('input[name="StepEnbaleThreshold"]:checked')?.value ?? null;
        ...
        function submit_step_ajax() {
            ...
            data.append("StepEnbaleThreshold", StepEnbaleThreshold);
            ...
            $.ajax({
                url: isEdit ? '?url=Steps/edit_step' : '?url=Steps/create_step',
                type: 'POST',
                data: data,
                processData: false,
                contentType: false,
                success: function (response) {
                    ...
                    success_response_seq(response, 'spinner', `../public/?url=Steps/index/${job_id}/${seq_id}`);
    ...
    function input_check() {
        ...
        const StepEnbaleThreshold = document.querySelector('input[name="StepEnbaleThreshold"]:checked')?.value ?? "0";
    ...
    function checkAndDisableDownshiftIfNotLastStep() {
        ...
        if (job_id && seq_id && step_id) {
            $.post("?url=Steps/check_step_is_last", {
                jobid: job_id,
                seqid: seq_id,
                stepid: step_id
            }, function (response) {
