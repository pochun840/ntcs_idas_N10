...
function edit_input_id() {
    ...
    // 防止外部變數不存在 - Fángzhǐ wàibù biàn shǔ bù cúnzài
...
// hiển thị thông báo khi row chưa được chọn
function requireJobSelected(input_event) {
    if (!input_event) {
        alertify.alert(pleaseSelectInputTitle, pleaseSelectInputMsg);
        return false;
    }
    return true;
}
...
function handleCopyJobEvent() {
    ...
    var from_job_name = jobinfo[job_id]['JOBname'];
    //const from_job_name = window.jobinfo?.[job_id]?.job_name || '';
    document.getElementById("from_job_id").value = job_id;
